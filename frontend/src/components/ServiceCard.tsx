...
export default function ServiceCard({ title, icon, delay = 0, customVisual, href }: ServiceCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      {href ? (
        <Link
          href={href}
          className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors text-sm font-semibold text-zinc-700 dark:text-zinc-200 shadow-md"
        >
          Learn More
          <ArrowRight size={16} />
        </Link>
