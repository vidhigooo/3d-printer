          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border p-6 bg-secondary/30">
              <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
                Cart <span className="text-sm font-normal text-muted-foreground">({cartCount} items)</span>
              </h2>
              <button
                onClick={() => setIsCartOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              {cartItems.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center text-muted-foreground space-y-4">
                  ...
                  <button 
                    onClick={() => setIsCartOpen(false)}
                    className="mt-4 px-6 py-2 rounded-full border border-border hover:bg-muted transition-colors text-foreground"
                  >
                    Continue Shopping
                  </button>
                </div>
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-border bg-background p-6">
                ...
                <Link
                  href="/cart"
                  onClick={() => setIsCartOpen(false)}
                  className="flex w-full items-center justify-center rounded bg-[#7a8a6b] px-6 py-4 text-sm font-medium tracking-wide text-white transition-colors hover:bg-[#6b795d] shadow-sm"
                >
                  View Cart
                </Link>
              </div>
