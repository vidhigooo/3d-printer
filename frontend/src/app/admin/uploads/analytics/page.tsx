...
function StatCard({
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-5 flex flex-col gap-3 backdrop-blur-sm">
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 pt-32 pb-20 px-6 selection:bg-cyan-500/30">
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <Link href="/admin/uploads" className="inline-flex items-center text-slate-400 hover:text-cyan-400 transition-colors mb-4 text-sm font-medium">
            ...
            <p className="text-lg text-slate-400">Visitor traffic and engagement for the last {range} days.</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-6 mb-6 backdrop-blur-sm">
              <h3 className="text-base font-semibold text-white mb-5">Daily Unique Visitors</h3>
                ...
                {daily.map((day) => {
                  ...
                  return (
                    <div key={day.date} className="group flex flex-col items-center flex-1 min-w-0 relative">
                      {/* Tooltip */}
                      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-800 text-cyan-400 text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[...daily].reverse().slice(0, 14).map((day, i) => (
                      <tr key={day.date} className={i % 2 === 0 ? "bg-slate-800/30" : ""}>
                        <td className="py-3 px-4 text-slate-400">{day.date}</td>
                        <td className="py-3 px-4 font-bold text-cyan-400">{day.uniqueVisitors}</td>
                        <td className="py-3 px-4 text-slate-300">{day.pageViews}</td>
                        <td className="py-3 px-4 text-slate-300">{day.uploads}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
